...
import { useCaptionOptions, useMediaPlayer, useVideoQualityOptions } from '@vidstack/react';
...
export function QualityOptions({
  ...
  const player = useMediaPlayer(),
  options = useVideoQualityOptions({ auto: true, sort: 'descending' }),
      ...
      >
        ...
        >
          {options.map(({ label, value, select}, index) => (
            <Radio value={value} onSelect={select} key={index}>
              {label}
            </Radio>
          ))}
        </DropdownMenu.RadioGroup>
